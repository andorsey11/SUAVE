...
def setup():
   ...
   # while takeoff_dif > 10:
    ...
    #pdb.set_trace()
     #takeoff_dif = procedure.post_process.takeoff_diff
...
def find_target_range(nexus,mission):
    ...
    cruise_range = mission.design_range-(x_climb_1+x_climb_5+x_descent_1+x_descent_2+x_descent_3+x_climb_3)
...
def design_mission(nexus):
    ...
    while abs(takeoff_error) > 1E-4: # Only want this sort of precision so the optimizer is happy

        nexus.results.base = mission.evaluate()
        max_takeoff_weight_guess = nexus.vehicle_configurations.takeoff.mass_properties.max_takeoff
        mission_fuelburn = max_takeoff_weight_guess -  results.base.segments['descent_3'].conditions.weights.total_mass[-1]
        actual_tow = nexus.vehicle_configurations.base.mass_properties.operating_empty + nexus.vehicle_configurations.base.passenger_weights.mass_properties.mass + mission_fuelburn + nexus.vehicle_configurations.base.mass_properties.breakdown.bag  + nexus.vehicle_configurations.base.mass_properties.cargo

        takeoff_error = (max_takeoff_weight_guess - actual_tow) # Needs a damping power to not overshoot?
        
        for config in nexus.vehicle_configurations:
            config.mass_properties.max_takeoff = max_takeoff_weight_guess - takeoff_error
            config.mass_properties.takeoff = max_takeoff_weight_guess - takeoff_error
        weight_cruise(nexus) # Recalculate all the aircraft weights with the new MTOW
        #print(takeoff_error)
        #print(nexus.vehicle_configurations.base.mass_properties.max_zero_fuel)
        #print(nexus.vehicle_configurations.base.mass_properties.operating_empty)
    #Now set the new MLW
    for config in nexus.vehicle_configurations:
...
def post_process(nexus):
    ...
    design_landing_weight    = vehicle.mass_properties.max_landing
    ...
    actual_takeoff_weight    = operating_empty+payload+summary.base_mission_fuelburn
    summary.takeoff_diff = (actual_takeoff_weight - design_takeoff_weight)**2
    summary.takeoff_weight = design_takeoff_weight
    ...
    summary.takeoff_weight  = max_takeoff_weight
    
    summary.takeoff_diff    = tofl_req - summary.takeoff_field_length
    summary.second_seg_diff = second_seg_grad_req - summary.second_seg_grad
    summary.fuel_margin_diff = summary.fuel_margin - fuel_margin_req
    summary.max_throttle_diff = summary.max_throttle - max_throttle_req
    summary.approach_speed_diff = summary.approach_Speed - approach_speed_req

    print(vehicle.propulsors['turbofan'].nacelle_diameter / Units.inches)
    print(vehicle.propulsors['turbofan'].engine_length / Units.inches)
    print(vehicle.propulsors['turbofan'].areas.wetted)
    print(vehicle.mass_properties.breakdown.propulsion)
    #print(vehicle.wings.main_wing.fuel_volume * .804 * 1000)
    import pdb; pdb.set_trace()
