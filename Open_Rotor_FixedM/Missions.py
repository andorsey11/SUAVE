...
def setup(analyses, vehicle):
    ...
    econ_mission = econ(analyses,vehicle)
    missions.econ = econ_mission
...
def base(analyses, vehicle):
  ...
  #  segment.air_speed    = cruise_speed * .8
    ...
    segment.atmosphere = atmosphere
    segment.planet     = planet
    cruise2_atmo       = airport.atmosphere.compute_values(cruise_altitude + vehicle.base.cruise_step,0)

    segment.air_speed  = cruise_speed * np.sqrt(cruise2_atmo.speed_of_sound / cruise_atmo.speed_of_sound)
    segment.distance   = 2050. * Units.nmi
    segment.state.numerics.number_control_points = tolerance
...
def econ(analyses, vehicle):
    ...
    #For MTOW
   # required_alt    = 44.3308 - 42.2665 * (cruise_atmo.density * (vehicle.base.mass_properties.econ_takeoff / vehicle.base.mass_properties.max_takeoff))**.234969
   # cruise_altitude = required_alt[0][0]*Units.km
   # vehicle.econ.cruise_altitude = cruise_altitude * Units.m
    #cruise_atmo     = airport.atmosphere.compute_values(required_alt[0][0],0)

    #For Econ FB
    required_alt    = mission.cruise_altitude
    cruise_atmo     = airport.atmosphere.compute_values(required_alt,0)
    ...
    # connect vehicle configuration
    segment.analyses.extend( analyses.econ )
   # segment.state.initials = vehicle.base.mass_properties.econ_takeoff
    # segment attributes
    segment.atmosphere   = atmosphere
    segment.planet       = planet
    segment.altitude_start = 1500  * Units.ft
  ...
  #  segment.air_speed    = cruise_speed * .8
    ...
    segment.atmosphere   = atmosphere
    segment.planet       = planet
    segment.altitude_end = cruise_altitude + vehicle.econ.cruise_step 
    climb_atmo = airport.atmosphere.compute_values(segment.altitude_end - vehicle.econ.cruise_step,0)
    #air_speed_ratio 
    segment.air_speed    = cruise_speed
  #  segment.air_speed    = cruise_speed * .8
    ...
    segment.atmosphere = atmosphere
    segment.planet     = planet
    cruise2_atmo       = airport.atmosphere.compute_values(cruise_altitude + vehicle.econ.cruise_step,0)
