...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotor'].bypass_factor <= 1 and config.propulsors['openrotor'].bypass_factor > 0:
            config.propulsors['openrotor'].bypass_ratio = 3.86*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9495) * config.propulsors['openrotor'].bypass_factor
        else:
            config.propulsors['openrotor'].bypass_ratio = 3.86*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9495)
