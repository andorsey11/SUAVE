...
def design_mission(nexus):
    ...
    mission.design_range = 4000 *Units.nautical_miles
    ...
    mission.design_range = 1333.3333333333333 *Units.nautical_miles
    find_target_range(nexus,mission)
    results = nexus.results
...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotor'].bypass_factor <= 1 and config.propulsors['openrotor'].bypass_factor > 0:
            config.propulsors['openrotor'].bypass_ratio = .988*4.013*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.944) * config.propulsors['openrotor'].bypass_factor
        else:
            config.propulsors['openrotor'].bypass_ratio = .988*4.013*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.944)
