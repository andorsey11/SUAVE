...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotor'].bypass_factor <= 1 and config.propulsors['openrotor'].bypass_factor > 0:
            config.propulsors['openrotor'].bypass_ratio = .98*4.05*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9437) * config.propulsors['openrotor'].bypass_factor
        else:
            config.propulsors['openrotor'].bypass_ratio = .98*4.05*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9437)
