        ...
        #copy the seed files to the new folder
            ...
            # Now change things in procedure to match. Only thing we need to change is the design range line
                ...
                if(design_range == 1000):
                    procedure_data[130] = "            config.propulsors['openrotor'].bypass_ratio = .98*4.051*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9541) * config.propulsors['openrotor'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotor'].bypass_ratio = .98*4.051*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9541)\n"
                    ...
                    procedure_data[130] = "            config.propulsors['openrotor'].bypass_ratio = 3.77*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.951) * config.propulsors['openrotor'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotor'].bypass_ratio = 3.77*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.951)\n"
                    ...
                    procedure_data[130] = "            config.propulsors['openrotor'].bypass_ratio = .986*3.9735*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.94167) * config.propulsors['openrotor'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotor'].bypass_ratio = .986*3.9735*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.94167)\n"
                elif(design_range == 3000):
                    procedure_data[130] = "            config.propulsors['openrotor'].bypass_ratio = .98*4.05*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9437) * config.propulsors['openrotor'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotor'].bypass_ratio = .98*4.05*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.9437)\n"
                elif(design_range == 4000):
                    procedure_data[130] = "            config.propulsors['openrotor'].bypass_ratio = .988*4.013*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.944) * config.propulsors['openrotor'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotor'].bypass_ratio = .988*4.013*(config.propulsors['openrotor'].fan.pressure_ratio-1)**(-.944)\n"
...
def header_vehicle_write(vehicle_file,payload,design_range):
             ...
             vehicle_file.write("    vehicle.tag = 'OR_Wing_Mounted" + str(payload) + "_" + str(design_range)+"'\n\n")    
             return vehicle_file
...
def engine_write(vehicle_file,payload,design_range):
  # ------------------------------------------------------------------
    ...
    #initialize the gas turbine network
            ...
            vehicle_file.write("    gt_engine.tag               = 'openrotor'\n\n")
