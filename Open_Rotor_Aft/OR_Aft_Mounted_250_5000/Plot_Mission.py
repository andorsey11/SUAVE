...
#                [ 0 , 0 , 0 ,0 ,0 , "%.5f" % parasitetotal[0][0]]]
   ...
   # f_write_config.write("MZFW                                       = " + "%.0f" % (vehicle.mass_properties.max_zero_fuel[0] * 2.20462) + "  lbm \n")
    ...
    f_write_config.write("Engine Type                                = " + str(vehicle.propulsors['openrotoraft'].tag) + "\n")
    f_write_config.write("Engine Thrust (M0.25)                      = " + "%.0f" % ((vehicle.propulsors['openrotoraft'].thrust.design_thrust*.2248) / vehicle.propulsors['openrotoraft'].number_of_engines) + "  lbf\n")
    f_write_config.write("Number of Engines                          = " + str(vehicle.propulsors['openrotoraft'].number_of_engines) + " \n")
    f_write_config.write("Engine Bypass Ratio                        = " + str(vehicle.propulsors['openrotoraft'].bypass_ratio) +  "\n")
    f_write_config.write("Nacelle Diameter                           = " + "%.1f" % (nexus.summary.nacelle_d)+ " inches\n")
    f_write_config.write("Fan Diameter                               = " + "%.1f" % (vehicle.propulsors['openrotoraft'].fan_diameter * 39.37)+ " Inches\n")
    f_write_config.write("Fan Pressure Ratio                         = " + str(vehicle.propulsors['openrotoraft'].fan.pressure_ratio) +  "\n")
    f_write_config.write("Thrust to Weight (SLS)                     = " + "%.3f" % ((vehicle.propulsors['openrotoraft'].sealevel_static_thrust*.2248 * vehicle.propulsors['openrotoraft'].number_of_engines) / (vehicle.mass_properties.max_takeoff*2.20462)) +  "\n\n")
