...
def base_setup():
    ...
    vehicle.tag = 'OR_Aft_Mounted50_3000'

    vehicle.mass_properties.max_takeoff = 67692.30769230769 * Units.lbs 
    vehicle.mass_properties.takeoff  = 67692.30769230769 * Units.lbs 
    vehicle.mass_properties.operating_empty = 35538.46153846153 * Units.lbs 
    vehicle.mass_properties.max_zero_fuel  = 46538.46153846153 * Units.lbs 
    vehicle.mass_properties.cargo = 0.  * Units.kilogram
    vehicle.mass_properties.econ_takeoff = 54153.846153846156 * Units.lbs 
    vehicle.envelope.ultimate_load = 2.5
    vehicle.envelope.limit_load    = 1.5
    vehicle.reference_area         = 647.693817468106  * Units['feet**2']
    vehicle.passengers             = 50
    vehicle.systems.control        = "fully powered" 
    vehicle.systems.accessories    = "medium range" 
    vehicle.cruise_altitude    = 30008.547008547008 
    vehicle.cruise_step        = 2000 / 3.28 * Units.m 
    vehicle.cruise_mach        =0.7787499999999999 
    ...
    landing_gear.nose_wheels = 1.0
    landing_gear.main_strut_length  = 52.53235387995386 * Units.inches
    landing_gear.nose_strut_length  = 52.53235387995386 * Units.inches
    landing_gear.main_tire_diameter = 33.86452460983874 * Units.inches
    landing_gear.nose_tire_diameter = 26.230555630191496 * Units.inches
    vehicle.landing_gear = landing_gear


    wing = SUAVE.Components.Wings.Main_Wing()
    wing.tag = 'main_wing'
    wing.areas.reference         = 647.693817468106 * Units['feet**2']
    ...
    wing.origin                  = [12.322047999999999,0,-0.5638799999999999]# meters
    wing.vertical                = False
    wing.symmetric               = True
    wing.high_lift               = True
    ...
    wing.areas.reference         = 129.5387634936212 * Units['feet**2']
    wing.aspect_ratio            = 1
    ...
    wing.twists.tip              = 0.0 * Units.degrees
    wing.origin                  = [21.820293333333332,0,1.9171919999999996]
    wing.vertical                = True
    ...
    wing.areas.reference         = 161.9234543670265 * Units['feet**2']
    wing.aspect_ratio            = 6  
    ...
    wing.span_efficiency         = 0.9
    wing.spans.projected         = numpy.sqrt(wing.aspect_ratio * wing.areas.reference)
    wing_origin_x                = vehicle.wings['vertical_stabilizer'].origin[0] + vehicle.wings['vertical_stabilizer'].spans.projected * numpy.tan((vehicle.wings['vertical_stabilizer'].sweeps.quarter_chord))
    wing_origin_z                = vehicle.wings['vertical_stabilizer'].origin[2] + vehicle.wings['vertical_stabilizer'].spans.projected
    wing.origin                  = [wing_origin_x,0,wing_origin_z] # meters
    wing.chords.root             = wing.yehudi_factor * 2 * wing.areas.reference / wing.spans.projected / (1+wing.taper)
    wing.chords.tip              = wing.chords.root * wing.taper
    wing.chords.mean_aerodynamic = wing.chords.root - (2*(wing.chords.root-wing.chords.tip)*(.5*wing.chords.root+wing.chords.tip)/(3*(wing.chords.root+wing.chords.tip)))
    wing.origin                  = [23.103839999999998,0,1.6916399999999998]
    wing.vertical                = False
    wing.symmetric               = True
    ...
    fuselage.lengths.nose            = 4.5110399999999995 * Units.meter 
    fuselage.lengths.tail            = 6.766559999999999 * Units.meter 
    fuselage.lengths.cabin           = 14.393333333333333 * Units.meter 
    fuselage.lengths.total           = 25.67093333333333 * Units.meter 
    fuselage.lengths.fore_space      = 3.608832 * Units.meter 
    fuselage.lengths.aft_space       = 5.413248 * Units.meter 
    fuselage.width                   = 2.2555199999999997 * Units.meter
    fuselage.heights.maximum         = 2.2555199999999997 * Units.meter
    fuselage.effective_diameter      = 2.2555199999999997 * Units.meter 
    ...
    fuselage.heights.at_quarter_length = 2.2555199999999997 * Units.meter
    fuselage.heights.at_three_quarters_length = 2.2555199999999997 * Units.meter
    fuselage.heights.at_wing_root_quarter_chord = 2.2555199999999997* Units.meter
    ...
    fan_nozzle.polytropic_efficiency = 1
    fan_nozzle.pressure_ratio        = 1
    ...
    fan.polytropic_efficiency = 0.788
    ...
    thrust.total_design             = 21575.237511492494 * Units.lbf
...
def configs_setup(vehicle):
    ...
    config.tag = 'econ'
    config.mass_properties.takeoff = 54153.846153846156* Units.lbs

    config.cruise_altitude = 25000 * Units.ft
