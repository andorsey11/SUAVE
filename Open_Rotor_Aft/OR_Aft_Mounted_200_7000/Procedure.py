...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotoraft'].bypass_factor <= 1 and config.propulsors['openrotoraft'].bypass_factor > 0:
            config.propulsors['openrotoraft'].bypass_ratio = 3.77*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.951) * config.propulsors['openrotoraft'].bypass_factor
        else:
            config.propulsors['openrotoraft'].bypass_ratio = 3.77*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.951)
