...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotoraft'].bypass_factor <= 1 and config.propulsors['openrotoraft'].bypass_factor > 0:
            config.propulsors['openrotoraft'].bypass_ratio = .988*4.013*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.944) * config.propulsors['openrotoraft'].bypass_factor
        else:
            config.propulsors['openrotoraft'].bypass_ratio = .988*4.013*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.944)
