...
def main():
    ...
    range_array   = [2000]#, 2000, 3000, 4000, 5000, 6000, 7000]
    ...
    for payload in payload_array:
        ...
        #copy the seed files to the new folder
            ...
            # Now change things in procedure to match. Only thing we need to change is the design range line
                ...
                if(design_range == 1000):
                    procedure_data[130] = "            config.propulsors['openrotoraft'].bypass_ratio = .98*4.051*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9541) * config.propulsors['openrotoraft'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotoraft'].bypass_ratio = .98*4.051*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9541)\n"
                ...
                elif(design_range == 3000):
                    procedure_data[130] = "            config.propulsors['openrotoraft'].bypass_ratio = .98*4.05*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9437) * config.propulsors['openrotoraft'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotoraft'].bypass_ratio = .98*4.05*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9437)\n"
                elif(design_range == 4000):
                    procedure_data[130] = "            config.propulsors['openrotoraft'].bypass_ratio = .988*4.013*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.944) * config.propulsors['openrotoraft'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotoraft'].bypass_ratio = .988*4.013*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.944)\n"
                elif(design_range == 5000):
                    procedure_data[130] = "            config.propulsors['openrotoraft'].bypass_ratio = .986*3.9735*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.94167) * config.propulsors['openrotoraft'].bypass_factor\n"
                    procedure_data[132] = "            config.propulsors['openrotoraft'].bypass_ratio = .986*3.9735*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.94167)\n"
                ...
                #DR 2000 = 4.0878*F16^(-0.9451147)*0.989
            ...
            #Optimization variables
                ...
                optimize_data[82] = "        [ 'fan_pressure_ratio'           , " + str(fan_pressure_guess)  +   "  ,   (1.05, 1.4)        ,     1.2         , Units.less],\n"
                ...
                if(design_range == 1000): # Restrict the econ mission so it doesnt fly backwards in cruise
                    ...
                    optimize_data[86] = "        #[ 'econ_cruise_step'             ,   1 / 3.28, (200  ,     4000)   ,     2000/3.28   , Units.m   ]\n" # Enforces basically no step
...
def header_vehicle_write(vehicle_file,payload,design_range):
             ...
             vehicle_file.write("    vehicle.tag = 'OR_Aft_Mounted" + str(payload) + "_" + str(design_range)+"'\n\n")    
             return vehicle_file
...
def top_level_write(vehicle_file,payload,design_range):
    ...
    # ------------------------------------------------------------------
            ...
            vehicle_file.write("    wing.t_tail                  = True\n")
   ...
   # ------------------------------------------------------------------        
    ...
    # ------------------------------------------------------------------        
            ...
            vehicle_file.write("    wing.span_efficiency         = 0.9\n")
            vehicle_file.write("    wing.spans.projected         = numpy.sqrt(wing.aspect_ratio * wing.areas.reference)\n")
            vehicle_file.write("    wing_origin_x                = vehicle.wings['vertical_stabilizer'].origin[0] + vehicle.wings['vertical_stabilizer'].spans.projected * numpy.tan((vehicle.wings['vertical_stabilizer'].sweeps.quarter_chord))\n")   
            vehicle_file.write("    wing_origin_z                = vehicle.wings['vertical_stabilizer'].origin[2] + vehicle.wings['vertical_stabilizer'].spans.projected\n")
            vehicle_file.write("    wing.origin                  = [wing_origin_x,0,wing_origin_z] # meters\n")
            vehicle_file.write("    wing.chords.root             = wing.yehudi_factor * 2 * wing.areas.reference / wing.spans.projected / (1+wing.taper)\n")   #A = .5 * [ ct + cr ] * s 
            vehicle_file.write("    wing.chords.tip              = wing.chords.root * wing.taper\n")
            vehicle_file.write("    wing.chords.mean_aerodynamic = wing.chords.root - (2*(wing.chords.root-wing.chords.tip)*(.5*wing.chords.root+wing.chords.tip)/(3*(wing.chords.root+wing.chords.tip)))\n")   #A-(2(A-B)(0.5A+B) / (3(A+B))) http://www.nasascale.org/p2/wp-content/uploads/mac-calculator.htm
...
def engine_write(vehicle_file,payload,design_range):
  # ------------------------------------------------------------------
    ...
    #initialize the gas turbine network
            ...
            vehicle_file.write("    gt_engine.tag               = 'openrotoraft'\n\n")
            ...
            vehicle_file.write("    gt_engine.scrubbed_chord    = vehicle.wings['main_wing'].chords.mean_aerodynamic\n\n")
    ...
    #Component 2 : inlet nozzle
            ...
            vehicle_file.write("    inlet_nozzle.polytropic_efficiency = 1\n")
            vehicle_file.write("    inlet_nozzle.pressure_ratio        = 1\n\n")
    ...
    #Component 8 :fan nozzle
            ...
            vehicle_file.write("    fan_nozzle.polytropic_efficiency = 1\n")
            vehicle_file.write("    fan_nozzle.pressure_ratio        = 1\n\n")
    ...
    #Component 9 : fan   
            ...
            # Fan vs DR including gearbox   0.803    0.796   0.788   0.780   0.771   0.761   0.751

            if (design_range == 1000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.803\n")
            elif (design_range == 2000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.796\n")
            elif (design_range == 3000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.788\n")
            elif (design_range == 4000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.780\n")
            elif (design_range == 5000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.771\n")
            elif (design_range == 6000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.761\n")
            elif (design_range == 7000):
                vehicle_file.write("    fan.polytropic_efficiency = 0.751\n")

            vehicle_file.write("    fan.pressure_ratio        = 1.1\n") # Design variable
            vehicle_file.write("    fan.spinner_ratio         = 2.5 \n")
            vehicle_file.write("    fan.nacelle_length_to_fan_di = 1.5\n\n")
