...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotoraft'].bypass_factor <= 1 and config.propulsors['openrotoraft'].bypass_factor > 0:
            config.propulsors['openrotoraft'].bypass_ratio = .98*4.051*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9541) * config.propulsors['openrotoraft'].bypass_factor
        else:
            config.propulsors['openrotoraft'].bypass_ratio = .98*4.051*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9541)
