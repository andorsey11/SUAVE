...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotoraft'].bypass_factor <= 1 and config.propulsors['openrotoraft'].bypass_factor > 0:
            config.propulsors['openrotoraft'].bypass_ratio = 3.86*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9495) * config.propulsors['openrotoraft'].bypass_factor
        else:
            config.propulsors['openrotoraft'].bypass_ratio = 3.86*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.9495)
