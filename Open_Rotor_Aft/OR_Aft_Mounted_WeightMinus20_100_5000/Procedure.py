...
def simple_sizing(nexus):
    ...
    for config in configs:
        if config.propulsors['openrotoraft'].bypass_factor <= 1 and config.propulsors['openrotoraft'].bypass_factor > 0:
            config.propulsors['openrotoraft'].bypass_ratio = .986*3.9735*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.94167) * config.propulsors['openrotoraft'].bypass_factor
        else:
            config.propulsors['openrotoraft'].bypass_ratio = .986*3.9735*(config.propulsors['openrotoraft'].fan.pressure_ratio-1)**(-.94167)
